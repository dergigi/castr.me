...
export class PodcastFeedGenerator {
  ...
  generateFeed(profile: NostrProfile, events: NDKEvent[], npub: string, longFormMap?: Map<string, NDKEvent>): string {
    ...
    // Generate value tag for channel-level default (use lnaddress if available)
...
<rss version="2.0" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:media="http://search.yahoo.com/mrss/" xmlns:podcast="https://podcastindex.org/namespace/1.0">
  ...
  private generateValueSplitsForEventSync(event: NDKEvent, longFormMap?: Map<string, NDKEvent>): ValueSplit[] {
    ...
    if (longFormMap) {
      ...
      if (longFormEvent) {
        const longFormSplits = this.nostrService?.extractValueSplitFromEvent(longFormEvent) || new Map()
        if (longFormSplits.size > 0) {
          return Array.from(longFormSplits.entries()).map(([pubkey, percentage]) => ({
            pubkey,
            percentage
          }))
        }
    ...
    const kind1Splits = this.nostrService?.extractValueSplitFromEvent(event) || new Map()
    if (kind1Splits.size > 0) {
      return Array.from(kind1Splits.entries()).map(([pubkey, percentage]) => ({
        pubkey,
        percentage
      }))
    }
  ...
  private async generateValueSplitsForEventAsync(event: NDKEvent, longFormMap?: Map<string, NDKEvent>, profile?: NostrProfile, npub?: string): Promise<ValueSplit[]> {
    ...
    if (longFormMap) {
      ...
      if (longFormEvent) {
        const longFormSplits = this.nostrService?.extractValueSplitFromEvent(longFormEvent) || new Map()
        if (longFormSplits.size > 0) {
          const profiles = await this.nostrService?.fetchZapProfiles(longFormEvent) || new Map()
          return Array.from(longFormSplits.entries()).map(([pubkey, percentage]) => ({
            pubkey,
            percentage,
            name: profiles.get(pubkey)?.name,
            lightningAddress: profiles.get(pubkey)?.lud16
          }))
        }
      }
    }
    ...
    const kind1Splits = this.nostrService?.extractValueSplitFromEvent(event) || new Map()
    if (kind1Splits.size > 0) {
      const profiles = await this.nostrService?.fetchZapProfiles(event) || new Map()
      return Array.from(kind1Splits.entries()).map(([pubkey, percentage]) => ({
        pubkey,
        percentage,
        name: profiles.get(pubkey)?.name,
    ...
    // Priority 3: Use profile defaults as 100% (use lnaddress if available)
    if (profile && npub && profile.lud16) {
      return [{
        pubkey: npub,
        percentage: 100,
